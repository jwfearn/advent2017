defmodule D05 do
  def escape(lines, fun \\ &increment/1) when is_binary(lines) do
    lines
    |> String.split
    |> Enum.reduce({}, &Tuple.append(&2, String.to_integer(&1)))
    |> escape(0, 0, fun)
  end
  def escape(tuple, cnt, pc, _) when pc >= tuple_size(tuple), do: cnt
  def escape(tuple, cnt, pc, fun) do
    jump = tuple |> elem(pc)
    tuple
  ...
  def strange(n) when n >= 3, do: n - 1
  def strange(n), do: n + 1
end
